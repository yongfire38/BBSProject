 ...
 */
package egovframework.com.utl.wed.util;
...
public class CkImageSaver {
	private static final Log log = LogFactory.getLog(CkImageSaver.class);
	 ...
	 */
	public CkImageSaver(String imageBaseDir, String imageDomain, String[] allowFileTypeArr, String saveManagerClass, EgovEnvCryptoServiceImpl cryptoService) {
	 ...
	 * @throws IOException
	 */
	public void saveAndReturnUrlToClient(HttpServletRequest request, HttpServletResponse response, MultipartFile file) throws IOException {
		...
		// Parse the request
		try {
			if (isAllowFileType(FilenameUtils.getName(file.getOriginalFilename()))) {
				String uploadFilePath = fileSaveManager.saveFile(file, imageBaseDir);
				...
				relUrl = "/utl/wed/imageSrc?"
					    + "path=" + this.encrypt(filePath)
					    + "&physical=" + this.encrypt(fileName)
					    + "&contentType=" + this.encrypt(file.getContentType());
