...
import egovframework.com.cop.bbs.bbs.service.EgovOpenSearchManageService;
...
public class EgovOpenSearchManageServiceImpl extends EgovAbstractServiceImpl implements EgovOpenSearchManageService {
	...
	public void createTextIndex() throws IOException {
		...
		List<String> stopTags = EgovStringUtil.readWordsFromFile(stopTagsPath);
		...
		// 동의어로 처리될 단어를 열거한다.
		List<String> synonym = EgovStringUtil.readWordsFromFile(synonymsPath);
		...
		// 분할처리하면 안되는 단어를 열거한다.
		List<String> userDictionaryRules = EgovStringUtil.readWordsFromFile(dictionaryRulesPath);
	...
	public void insertTotalData() {
	    ...
	    while (startIndex < totalSize) {
	        ...
	        batchList.forEach(bbsArticleInfo -> {
	            ...
	            dataMap.put("nttSj", EgovStringUtil.cleanString(bbsArticleInfo.getNttSj()));
	            dataMap.put("nttCn", EgovStringUtil.cleanString(bbsArticleInfo.getNttCn()));
	...
	public void insertTotalEmbeddingData() {
		...
		for (int i = 0; i < totalPages; i++) {
			...
			splittedBbsArticleInfoList.stream().map(bbsArticleInfo -> {
				...
				Embedding bbsArticleResponse = embeddingModel.embed(EgovStringUtil.cleanString(bbsArticleInfo.getNttSj() + " " + bbsArticleInfo.getNttCn())).content();
